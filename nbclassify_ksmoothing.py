...
def classify_enhanced():
    ...
    for root, subdir, subfiles in os.walk(directory):
        ...
        for file in files:
            ...
            probab_ham_words = 0
            for w in words:
                if w not in stopwords.stopwords:
                    # Added one smoothing
                    if w in data["word_dict"]:
                        # Add k smoothing
                        probab_ham_words += math.log((data["word_dict"][w]["ham_count"] +
                                                      (m/data["vocab_size"]))/(data["ham_wcount"] + m))
                        probab_spam_words += math.log((data["word_dict"][w]["spam_count"] +
                                                       (m/data["vocab_size"]))/(data["spam_wcount"] + m))
...
if __name__ == "__main__":classify_enhanced()